...
class CdpConnection {
  ...
  private constructor(private readonly socket: NodeWebSocket,private readonly sendTimeoutMs:number,private readonly lifecycleSignal?:AbortSignal) {
    this.abortListener=()=>this.failAll(abortError('CDP lifecycle aborted'));
...
export class DashboardBrowser {
  ...
  async waitFor(expression: string, timeoutMs = 8_000): Promise<void> {
...
async function removeDirectory(path:string):Promise<void>{const root=resolve(tmpdir());const target=resolve(path);if(!target.startsWith(`${root}\\`)||!target.split(/[\\/]/).pop()?.startsWith('thoth-dashboard-browser-'))throw new Error(`Refusing to remove unvalidated browser profile: ${target}`);let lastError:unknown;for(let attempt=0;attempt<60;attempt+=1){try{rmSync(target,{recursive:true,force:true});return;}catch(error){lastError=error;await delay(50);}}throw lastError;}
async function readDevToolsPort(path:string,signal:AbortSignal):Promise<string>{const started=Date.now();while(Date.now()-started<8_000){throwIfAborted(signal);try{if(existsSync(path)){const [port]=readFileSync(path,'utf8').trim().split(/\r?\n/);if(/^\d+$/.test(port))return port;}}catch{/* Chrome may still own the new file briefly on Windows. */}await bounded(delay(25),100,'DevTools port retry',signal);}throw new Error('Timeout waiting for readable Chrome DevTools port');}
...
async function terminateBrowser(chrome:ChildProcess|null):Promise<void>{if(!chrome||chrome.exitCode!==null)return;const exited=new Promise<void>((resolveExit)=>chrome.once('exit',()=>resolveExit()));chrome.kill();try{await bounded(exited,3_000,'Chrome exit');}catch{chrome.kill('SIGKILL');await bounded(exited,2_000,'forced Chrome exit');}}
...
export const harnessFaultTestApi={
  ...
  processExists:(pid:number)=>{try{process.kill(pid,0);return true;}catch{return false;}},
