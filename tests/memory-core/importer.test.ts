...
describe('one-way legacy importer', () => {
  it('keeps the supported source read-only and imports only authoritative rows with quarantine', () => {
    ...
    try {
      ...
      expect(report).toMatchObject({ reportVersion: 2, sourceSchemaVersion: 'legacy-v1', targetSchemaVersion: SQLITE_SCHEMA_REVISION, committed: true, dispositions: { sessions: { imported: 1, quarantined: 1 }, prompts: { imported: 1, quarantined: 1 }, observations: { imported: 1, skipped: 1 } } });
      ...
      expect(targetDb.prepare('SELECT max(version) AS version FROM schema_migrations').get()).toEqual({ version: SQLITE_SCHEMA_REVISION });
      targetDb.close();
