...
export function getOpenApiSpec(port: number): Record<string, unknown> {
  return {
    ...
    paths: {
      ...
      '/viz/atlas': {
        get: {
          summary: 'Navigate the semantic memory atlas by Universe, Community, or Neighborhood',
          parameters: [
            { name: 'level', in: 'query', schema: { type: 'string', enum: ['universe', 'community', 'neighborhood'], default: 'universe' } },
            { name: 'project_token', in: 'query', schema: { type: 'string' } },
            { name: 'session_token', in: 'query', schema: { type: 'string' } },
            { name: 'topic_token', in: 'query', schema: { type: 'string' } },
            { name: 'type', in: 'query', schema: OBSERVATION_TYPE_SCHEMA },
      ...
      schemas: {
        ...
        SemanticAtlasError: {
          ...
          properties: {
            ...
            retryable: { type: 'boolean' },
            recover_to_level: { type: 'string', enum: ['universe', 'community', 'neighborhood'] },
          },
          required: ['error', 'code', 'retryable'],
        },
        ...
        ObservatoryRecallHit: {
          ...
          required: ['observation_id', 'title', 'preview', 'type', 'project', 'session', 'topic', 'community_id', 'created_at', 'lane', 'pivot_token'],
        },
        ...
        ObservatoryPivotResponse: {
          ...
          required: ['context_token', 'scope', 'focus_node_id', 'community_id', 'target'],
        },
        ...
        SemanticAtlasNode: {
          ...
          required: ['id', 'kind', 'label', 'snippet', 'project', 'session', 'topic', 'type', 'community_id', 'member_count', 'project_count', 'unclustered', 'seed_x', 'seed_y'],
        },
        SemanticAtlasEdge: {
          type: 'object',
          properties: {
        ...
        SemanticAtlasNavigation: {
          ...
          required: ['community_id', 'focus_node_id', 'depth', 'region_id', 'source_memory_count', 'visible_memory_count', 'source_relationship_count', 'visible_relationship_count', 'represented_source_relationship_count', 'omitted_nodes', 'omitted_edges', 'raw_rich_render_safe', 'raw_rich_render_limit', 'scope'],
        },
        SemanticAtlasPageResponse: {
          type: 'object',
          properties: {
            level: { type: 'string', enum: ['universe', 'community', 'neighborhood'] },
          ...
          required: ['level', 'generation', 'presentation', 'nodes', 'edges', 'regions', 'region_bridges', 'counts', 'coverage', 'facets', 'navigation', 'continuation', 'truncated', 'health'],
        },
        VizSliceResponse: {
          type: 'object',
          properties: {
