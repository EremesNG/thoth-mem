...
export const MEMORY_PROTOCOL_SECTIONS = {
  ...
  ownership: [
    ...
    'When native enrollment and prompt capture are confirmed, do not repeat the same start or prompt write manually.',
    'When either native effect is degraded or unsupported, use its explicit manual fallback only with the stable root session ID and project name, then confirm MCP success.',
    ...
    'Resolve the stable root session ID before memory work, and use the current project name.',
    "The root session ID is the harness's stable identifier for the root session, regardless of the native field or runtime label that carries it.",
    'The project name is the name of the project being worked on; when it is not explicit, derive it from the repository or workspace directory name.',
    'For mem_session, parameter id receives the root session ID and parameter project receives the project name.',
    'For other memory tools, parameter session_id receives the same root session ID.',
    'If the stable root session ID is unavailable, expose the limitation instead of inventing continuity.',
  ].join('\n'),
  lifecycle: [
    ...
    'A semantic session summary is agent-owned: persist it when meaningful root work is actually complete, without waiting for a terminal hook.',
    'Do not treat turn-scoped Stop or process-exit callbacks as semantic completion.',
