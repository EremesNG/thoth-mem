...
export function noOp(reason: string, intent?: LifecycleIntent): AdapterEventResult {
  return {
    action: 'return',
    outcome: 'no_op',
    retryable: false,
...
export function supported(trigger: string): Capability {
  return { state: 'supported', trigger };
}

export function incomplete(trigger: string, reason: string): Capability {
  return { state: 'degraded', trigger, reason };
}

export function unsupported(reason: string): Capability {
