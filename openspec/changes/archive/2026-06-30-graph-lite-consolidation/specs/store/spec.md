...
> Sub-change B1 of Change B. Adds the KG-backed `ObservationFact` adapter,
> migrates every `observation_facts` reader/writer onto it, repoints the rebuild
> plumbing, and removes the `observation_facts` table + indexes via an ordered,
> reversible migration. The portable sync/export format is unchanged.
...
- WHEN facts are read through the adapter and (pre-removal) through the legacy
  reader
- THEN the set of observations that contribute at least one fact MUST be the same
...
- THEN the table MUST be recreatable and repopulatable from observations with no
  data loss (because its contents are fully derivable)
