...
- The npm tarball is the authoritative release artifact for setup and runner smoke testing; source-tree success alone is insufficient.
- Node.js 18 or newer is available because it is the existing package runtime floor.
- Every version-bearing harness manifest equals the packed `package.json` version exactly; ranges and alternate compatibility representations are rejected.
- The canonical inventory has one unique package-relative path per entry with exactly one harness owner and one role; runtime declarations and tarball contents must reconcile against it.
- Path containment is checked on both normalized lexical paths and resolved real paths so links cannot escape the package or marketplace root.
- Marketplace and plugin validation are exercised locally against packaged fixtures; publishing, remote marketplace mutation, and credential use remain out of scope.
- Project-scoped Codex setup may still attempt explicitly requested CLI-level marketplace/plugin registration when that operation is inherently global, but the result must identify that external scope before mutation.

## Handoff Hints

- Design a single authoritative inventory consumed by package-content and packed-install verification so manifest declarations cannot drift from tarball assertions.
- Resolve all runtime paths from the installed package or marketplace root and exercise them with inaccessible source checkouts.
- Include Windows and POSIX path-with-spaces fixtures for every runner shape used by a harness.
- Validate version and plugin identity consistency before invoking any hook in smoke tests.
- Preserve exact package-version equality, canonical inventory cardinality, and lexical-plus-realpath containment checks.
- Keep remote publishing and user credentials outside implementation and verification.
