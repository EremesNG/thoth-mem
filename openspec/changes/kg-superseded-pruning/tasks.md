...
- [ ] 1.1 Add three C1 config knobs to `KnowledgeGraphConfig`, defaults, and env resolver — `src/config.ts`
  **[USN-1]** | Priority: P1
...
- [ ] 1.2 Document three C1 knobs in `config.schema.json` — `config.schema.json`
  **[USN-1]** | Priority: P1
...
- [ ] 1.3 Add additive composite index `idx_kg_triples_slot_superseded` (fresh-DB schema) — `src/store/schema.ts`
  **[USN-2]** | Priority: P1
...
- [ ] 1.4 Idempotently ensure `idx_kg_triples_slot_superseded` on existing DBs — `src/store/migrations.ts`
  **[USN-2]** | Priority: P1
  **Spec:** `knowledge-graph/Bounded Retention MUST Keep Only the N Most-Recent Superseded Triples Per Slot`
...
- [ ] 2.1 Add `PruneSupersededTriplesInput` / `PruneSupersededTriplesResult` types — `src/store/types.ts`
  **[USN-3]** | Priority: P1
...
- [ ] 2.2 **CORE TASK** — Implement plain shared core `runSupersededPrune(db, opts)` — `src/store/index.ts`
  **[USN-3]** | Priority: P1
...
- [ ] 2.3 Implement transactional entry point `store.pruneSupersededTriples(input)` — `src/store/index.ts`
  **[USN-3]** | Priority: P1
...
- [ ] 2.4 Add `subject_entity_id` to the B3 prior-rows SELECT — `src/indexing/jobs.ts`
  **[USN-4]** | Priority: P1
...
- [ ] 2.5 **CORE TASK** — Wire automatic incremental enforcement into `persistKgExtraction` — `src/indexing/jobs.ts`
  **[USN-4]** | Priority: P1
...
- [ ] 2.6 Implement CLI `prune-graph` command — `src/cli.ts`
  **[USN-5]** | Priority: P1
...
- [ ] 2.7 Implement HTTP `POST /graph/prune` route — `src/http-routes.ts`
  **[USN-5]** | Priority: P1
...
- [ ] 2.8 Register the `/graph/prune` route and import the handler — `src/http-server.ts`
  **[USN-5]** | Priority: P1
...
- [ ] 2.9 Document `/graph/prune` in the OpenAPI spec — `src/http-openapi.ts`
  **[USN-5]** | Priority: P2
  **Spec:** `indexing/prune-graph MUST Be a CLI + HTTP Admin Op, Not an MCP Tool`
...
- [ ] 2.10 Add keep-N retention eval case + OFF-vs-ON no-regression gate — `src/evals/retrieval.ts`
  **[USN-6]** | Priority: P1
...
- [ ] 3.1 Write keep-N boundary tests (N+k, exactly N, N+1, keep-N=0) — `tests/store/kg-prune.test.ts` (new)
  **[USN-7]** | Priority: P1
...
- [ ] 3.2 Write tie-on-`superseded_at`, current-never-pruned, and determinism/idempotency tests
  **[USN-7]** | Priority: P1
...
- [ ] 3.3 Write all-or-nothing transactional failure test — `pruneSupersededTriples`
  **[USN-7]** | Priority: P1
...
- [ ] 3.4 Write project-scope and dry-run/real-equivalence tests
  **[USN-7]** | Priority: P1
...
- [ ] 3.5 Write dangling-ref-NULLing and orphan-cleanup on/off tests
  **[USN-7]** | Priority: P1
...
- [ ] 3.6 Write delete-path non-interference test
  **[USN-7]** | Priority: P2
...
- [ ] 3.7 Write automatic-path steady-state cap and slot-scoping tests — `tests/indexing/kg-prune-automatic.test.ts` (new) or extend `tests/indexing/jobs.test.ts`
  **[USN-8]** | Priority: P1
...
- [ ] 3.8 Write automatic flag-off byte-identical and supersede-off-inert tests
  **[USN-8]** | Priority: P1
...
- [ ] 3.9 **CRITICAL TEST** — Write LLM double-write idempotency test
  **[USN-8]** | Priority: P1
...
- [ ] 3.10 Write migration idempotency test — `tests/store/migration.test.ts`
  **[USN-2]** | Priority: P1
  **Spec:** `knowledge-graph/Bounded Retention MUST Keep Only the N Most-Recent Superseded Triples Per Slot`
...
- [ ] 3.11 Write retrieval unaffected-by-pruning tests — `tests/store/retrieval.test.ts` or nearest recall suite
  **[USN-9]** | Priority: P1
...
- [ ] 3.12 Write retrieval flag-off byte-identical test
  **[USN-9]** | Priority: P1
...
- [ ] 3.13 Write MCP registry unchanged and `mem_project action=graph` unaffected tests — `tests/tools/mem-project.test.ts` or nearest tool-registration suite
  **[USN-10]** | Priority: P1
...
- [ ] 3.14 Write config resolution tests (env/persisted/default, keep-N=0, per-project override, schema accept/reject) — `tests/config.test.ts`
  **[USN-1]** | Priority: P1
...
- [ ] 3.15 Write CLI/HTTP dry-run-vs-real and no-op-when-supersession-off tests
  **[USN-5]** | Priority: P1
...
- [ ] 3.16 Write export/import parity test — `tests/store/export-import.test.ts`
  **[USN-11]** | Priority: P2
...
- [ ] 4.2 Eval gate: keep-N retention case + OFF-vs-ON no-regression comparison
  **[USN-12]** | Priority: P1
  **Spec:** `evals/Evals MUST Validate keep-N Retention Bounds Superseded Triples`; `evals/Eval Suite MUST Gate on No Retrieval Regression With Pruning Enabled`
...
- [ ] 4.4 Record shipped `kgPruneEnabled` default per the eval-gated rollout condition — `src/config.ts`, `openspec/changes/kg-superseded-pruning/checklists/requirements.md`
  **[USN-12]** | Priority: P1
...
- [ ] 4.5 Confirm retrieval, export/import, and MCP surface remain byte-identical post-rollout
  **[USN-12]** | Priority: P1
